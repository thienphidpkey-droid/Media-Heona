...
export const FloatingMenu: React.FC = () => {
  return (
    <div className="fixed bottom-5 left-0 right-0 z-[9999] flex justify-center lg:hidden pointer-events-none animate-slide-up px-4">
      <div className="bg-[#6f3aff]/20 backdrop-blur-xl rounded-full px-3 py-2 md:px-4 md:py-2.5 flex items-center gap-1.5 md:gap-2 border border-[#9d7aff]/30 shadow-[0_0_25px_rgba(111,58,255,0.35)] pointer-events-auto">
        {MENU_ITEMS.map((item) => (
          <NavLink
            key={item.path}
            to={item.path}
            className={({ isActive }) =>
              `group relative p-2.5 md:px-3.5 md:py-2 rounded-full flex items-center justify-center transition-all duration-300 ${
                isActive
                  ? 'bg-gradient-to-tr from-primary to-secondary text-white shadow-[0_0_20px_rgba(111,58,255,0.6)] scale-105'
                  : 'text-textMuted hover:bg-white/10 hover:text-white hover:scale-105'
              }`
            }
          >
            {item.icon}
